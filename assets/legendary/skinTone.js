...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function changeSkinTone(tone) {
        const avatarParts = document.querySelectorAll('#avatar-container [data-body-part]');
        if (avatarParts.length === 0) {
            console.error('Avatar parts not found');
            return;
        }
        ...
        avatarParts.forEach(part => {
            ...
            part.querySelectorAll(`[fill="${currentColors.main}"]`).forEach(el => {
                el.setAttribute('fill', newColors.main);
            });

            // Change shadow color
            part.querySelectorAll(`[fill="${currentColors.shadow}"]`).forEach(el => {
